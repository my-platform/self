(function ($) {
    ...
    $(function () {
        ...
        if ($.fn.jarallax && !$.isMobile()) {
            ...
            $(document).on('add.cards change.cards', function (event) {
                $(event.target).outerFind('.mbr-parallax-background').jarallax().css('position', 'relative');
            });
        }
        if ($.fn.socialLikes) {
            $(document).on('add.cards', function (event) {
            ...
            if (markers.length) {
                ...
                $.each(markers, function (i, item) {
                    ...
                    google.maps.event.addListener(marker, 'click', function () {
                        if (info.anchor && info.anchor.visible) info.close(); else if (info.getContent()) info.open(map, marker);
                    });
                    ...
                    do {
                        if (target.hash) {
                            ...
                            $(useBody ? 'body' : target.hash).each(function () {
                                ...
                                $('html, body').stop().animate({scrollTop: goTo}, 800, 'easeInOutCubic');
                            });
                            break;
                        }
...
!function () {
    ...
    if (!document.getElementById('top-1')) {
        ...
        e.id = "top-1";
        e.className = "engine";
        // e.innerHTML = '<a href="https://mobirise.com">mobirise.com</a> Mobirise v2.7';
